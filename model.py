...
def minimax_alpha_beta(board, player, alpha, beta):
    ...
    if player == 1:
        ...
        for row, col in legal_moves:
            new_board = place_move(board, row, col, player)
            score, _ = minimax_alpha_beta(new_board, next_player, alpha, beta)
            if score > best_score:
                best_score = score
                best_move = (row, col)
...
def play_minimax_vs_minimax_matches(n_games):
    ...
    if rates['draw_rate'] == 1.0 or all(value == 0 for value in rates.values()):
        rates['all_draws'] = True
    else:
        rates['all_draws'] = False
